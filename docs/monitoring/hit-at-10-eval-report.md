...
선호 회복률 격차(0.800 vs 0.780)도, 보정이 실제로 적용된 39명만
...
|---|---|---|---|---|
| ① 임베딩만 | 0.980 | 0.772 | 0.780 | 0.480 |
| ② 임베딩+메타데이터 (현재 프로덕션) | 1.000 | 0.997 | 0.995 | 0.780 |
| ③ 임베딩+메타데이터+보정모델 | 1.000 | 0.998 | 0.995 | 0.800 |
| ④ 단순추론 (LLM 직접 순위) | 1.000 | 0.947 | 0.939 | 0.480 |
...
| 비교군 | Hit@10 | NDCG@10 (LLM 정답 기준) | NDCG@10 (선호 시나리오 기준) | 선호 회복률 |
|---|---|---|---|---|
| ① 임베딩만 | 1.000 | 0.757 | 0.767 | 0.436 |
| ② 임베딩+메타데이터 (현재 프로덕션) | 1.000 | 0.996 | 0.994 | 0.744 |
| ③ 임베딩+메타데이터+보정모델 | 1.000 | 0.997 | 0.995 | 0.769 |
| ④ 단순추론 (LLM 직접 순위) | 1.000 | 0.938 | 0.930 | 0.462 |
...
|---|---|---|---|---|
| ② 임베딩+메타데이터 (현재 프로덕션) | 1.000 | 0.997 | 0.995 | 0.780 |
| ③′ 이상적 보정 (가상 시나리오, 실측 아님) | 1.000 | 0.993 | 0.995 | 1.000 |
...
**한편 NDCG(LLM 정답 기준)는 오히려 소폭 낮아진다(0.997 → 0.993)** — 이 지표는
...
- 모델: `gpt-5.6-luna`
- 호출 50회, 총 155775 토큰(input 147453, output 8322)
- 평균 응답 시간: 2.90초/호출
- 추정 비용(gpt-4.1-mini 참고 단가 기준, 실제 `gpt-5.6-luna` 단가와 다를 수 있음): $0.0723(요청 50건 기준, 후보군이 커지면 입력 토큰이 선형으로 늘어난다)
...
- 정답(ground truth)은 `gpt-5.6-terra`(`OPENAI_JUDGE_MODEL`)로 생성했다
